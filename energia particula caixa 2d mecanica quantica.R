# Makes a plot for Energy levels for a particle in a 2d box
# Elmar Uhl - 2024
...
# Value of sum from quantum numbers nx, ny
n <- 6
...
energy <- function(nx,ny) {
    A = (hbar**2*pi**2)/(2*m)
    y = A*(nx**2/a**2 + ny**2/b**2)
    return(y)
}

nx <- ny <- seq(1,n-1) # Vector with values that quantum numbers can be assignment
soma <- c() # Storage nx + ny values

nx2 <- ny2 <- c() # Will storage permitted values of nx + ny = n
for (i in nx) {
  for (ii in ny) {
    s = i + ii
    if (s <= n) { # Verify if nx + ny <= n and storage nx2, ny2 in afirmative case
        nx2 <- c(nx2,i)
        ny2 <- c(ny2,ii)
        soma <- c(soma, s)
    }
...
value_max <- max(soma)
print(paste("Maximum value of sum sequence (nx + ny): ", value_max))
value_min <- min(soma)
print(paste("Minimum value of sum sequence (nx + ny): ", value_min))

nx3 <- ny3 <- c() # Storage ascending order
...
for (i in seq(1, values_length) ){
  energia <- c(energia, energy(nx3[i],ny3[i]))
}
...
for (i in seq(value_min,value_max)) {
  ...
  for (ii in seq(1, values_length)) {
    if (soma3[ii] == i) {
      ...
      if (text_OnOff)
        text(x[1],y[1],paste('(',nx3[ii],',',ny3[ii], ')'),adj=c(0,-0.5),cex=1)
      x <- x + 3
    }
  }
}
